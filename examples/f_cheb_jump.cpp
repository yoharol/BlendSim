...
int main() {
  ...
  aphys::Scene scene = aphys::create_scene(
      ...
      aphys::create_camera(
          {-18.44f, 10.13f, 18.57f},            // camera position
          {0.0f, 3.0f, 0.0f},                   // camera target
  ...
  double devia_stiffness = 470.0f;
  double hydro_stiffness = 470.0f;
  ...
  add_gui_func(gui, [&]() {
    ...
    if (export_usd) {
      TrajectoryExporter exporter(trajectory, tm.tets, 3);
