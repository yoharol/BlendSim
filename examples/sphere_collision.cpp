...
int main() {
  ...
  std::string cnslash = "/";
  std::string assets_path = SIMTF_RESOURCES_PATH + cnslash + "sphere" + cnslash;
  {
    std::string ref_file = assets_path + "sphere.mesh";
  ...
  std::vector<int> n_samples_interval = {20, 0, 20};
  ...
  SplineTrajectory origin_trajectory;
  copy_trajectory(trajectory, origin_trajectory);
  ...
  /*{
    ...
    Vecxd new_arg_vec = Vecxd::Map(new_arg.data(), new_arg.size());
    retrive_from_argPV<3>(trajectory, arg_pv, new_arg_vec);
  }*/
  ...
  /*sum_hessian_constrained.conservativeResize(n_args * 3 + 1, n_args * 3 + 1);
  for (int i = 0; i < n_verts; i++) {
    int idx = arg_pv.keyframe_idx[1].P_idx_in_arg(i);
    sum_hessian_constrained.coeffRef(n_args * 3, idx * 3 + 1) = 1.0;
    sum_hessian_constrained.coeffRef(idx * 3 + 1, n_args * 3) = 1.0;
  }
  ...
  auto optimize_trajectory_PV = [&](int max_steps = 1) {
    ...
    while (!converged && step++ < max_steps) {
      ...
      if ((Wsp - prev_loss) * (Wsp - prev_loss) > 1e-2) {
        ...
        std::cout << "distance:"
                  << compute_traj_distance(trajectory, origin_trajectory)
                  << std::endl;
      } else {
        converged = true;
        break;
  ...
  auto optimize_trajectory_T = [&]() {
    ...
    Vecxd search_direct = result;
    trajectory.T_between[0] += 0.3 * result(0);
    trajectory.T_between[2] += 0.3 * result(1);
