...
int main() {
  ...
  for (int i = 0; i < top_n_bottom.size(); i++) {
    ...
    trajectory.keyframes[0].p_fix[idx] = 1;
    // trajectory.keyframes[0].v_fix[idx] = 1;
    trajectory.keyframes[1].p_fix[idx] = 1;
    // trajectory.keyframes[1].v_fix[idx] = 1;
    trajectory.keyframes[2].p_fix[idx] = 1;
    // trajectory.keyframes[2].v_fix[idx] = 1;
  }

  trajectory.keyframes[2].p_fix.fill(true);
  trajectory.keyframes[2].v_fix.fill(true);
  ...
  trajectory.keyframes[2].p_fix = trajectory.keyframes[0].p_fix;
  trajectory.keyframes[2].v_fix = trajectory.keyframes[0].v_fix;
  ...
  auto optimize_trajectory_PV = [&](int max_steps = 10) {
    ...
    while (!converged && step++ < max_steps) {
      ...
      std::cout << "total time:" << sum_vector(trajectory.T_between)
                << std::endl;
      print_tet_info(tm);
    }
  };
  ...
  auto optimize_trajectory_T = [&]() {
    ...
    for (int k = 0; k < n_intervals; k++) {
      LHS(k, k) = sum_H_T(k);
      rhs(k) = -sum_J_T(k);
      LHS(n_intervals, k) = k % 2 == 0 ? 1.0 : -1.0;
      LHS(k, n_intervals) = k % 2 == 0 ? 1.0 : -1.0;
    }
    rhs(n_intervals) = 0.0;
    Vecxd result = LHS.colPivHouseholderQr().solve(rhs);
    double tau = 0.0;
    Vecxd search_direc = result.head(n_intervals);
    // Vecxd search_direc = -sum_J_T.cwiseQuotient(sum_H_T);
  ...
  add_gui_func(gui, [&]() {
    ...
    if (save) {
      ...
      exporter.SaveAnimationToUSD(output_path + "stretch3D_retimed.usdc");
      SaveFrameAnimatinoToUsd(trajectory, tm.tets,
                              output_path + "stretch3D_frame.usdc", 60);
    }
  });
