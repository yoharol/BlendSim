...
int main() {
  ...
  reg.recompute_reference_deformationP(sample_batch, trajectory, pd_solver,
                                       tm.tets, newP);
  std::vector<Energy_Jacobian_Func> regularized_jacobian_funcs;
  for (int s = 0; s < sample_batch.n_samples; s++) {
    regularized_jacobian_funcs.push_back(
        [&pd_solver, s, &newP, &n_verts](const MatxXd& v_p, Vecxd& jacobian) {
          MatxXd gradient = pd_solver.L * v_p - pd_solver.J * newP[s];
          jacobian = Eigen::Map<Vecxd>(gradient.data(), n_verts * 3);
        });
  }
      ...
      if (optimize_time) {
        ...
        while (!converged) {
          ...
          aphys::apply_arg_in_solver<3>(arg_pv, trajectory, arg_pred,
                                        traj_solver);
          double Wsp = summarize_regularized_eularized_jacobian_PV_with_pre<3>(
              sample_batch, arg_pv, trajectory, vert_mass, arg_J, hessians,
              eulerized_hessians, regularized_jacobian_funcs, h, traj_solver);
