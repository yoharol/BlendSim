...
int main() {
  ...
  std::vector<int> segments = {2, 2, 1, 1, 1, 1, 1, 1, 1};
  ...
  std::vector<int> n_samples_interval = {5,  5,  5,  5,  10, 10,
                                         10, 10, 10, 10, 10};
  ...
  while (!glfwWindowShouldClose(window)) {
      ...
      if (optimize_time) {
        ...
        while (!converged) {
          ...
          double Wsp = summarize_damped_eularized_jacobian_PV_with_pre<3>(
              ...
              eulerized_hessians, pd_solver.energy_jacobian_func,
              damping_alpha);
    ...
    // compute_damped_force_residual<3>(f, jacobian, v_vel, v_acce, vert_mass,
    //                                  currT, damping_alpha);
    // f = -jacobian;
    // aphys::MatxXd f_mat = aphys::MatxXd::Map(f.data(), n_verts, 3);

    // set_deriv_lines_data(extern_force, v_p.cast<float>(),
    // f_mat.cast<float>(),
    //                      1.0, MatxXf());
