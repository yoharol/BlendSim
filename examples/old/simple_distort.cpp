...
int main() {
  ...
  GLFWwindow* window =
      aphys::create_window(SCR_WIDTH, SCR_HEIGHT, "Distort Optimize Example");
  Scene scene = create_scene(default_light, default_camera);
  set_2d_camera(scene.camera, 0.0f, 1.0f, 0.0f, 1.0f);

  std::string cnslash = "/";
  std::string assets_path =
      SIMTF_RESOURCES_PATH + cnslash + "distort" + cnslash;
  std::vector<MatxXd> key_frames(2);
  Matx3i face_indices;
  for (int i = 0; i < 2; i++) {
    std::string filename = assets_path + std::to_string(i) + ".obj";
    MatxXd v;
    igl::readOBJ(filename, v, face_indices);
    // convert to 2D
    key_frames[i] = v.leftCols(2);
  }
  std::vector<int> segments = {1};
  std::vector<double> time_interval = {1.0};
