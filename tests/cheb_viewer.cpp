...
int main() {
  ...
  lbs_model.load_anim_data(assets_path + "cheb_anim.dmat");
  lbs_model.initialize_lbs_weights(tm.verts);
  added_lbs_model.load_anim_data(assets_path + "cheb_added_anim.dmat");
  ...
  // 0 155 185 230 260 310 340 395 430 475
  Eigen::VectorXi keyframes(10);
  keyframes << 0, 155, 185, 230, 260, 310, 340, 395, 430, 475;
  MatxXd selected_frames(keyframes.size(), lbs_model.anim_frame_data.cols());
  for (int i = 0; i < keyframes.size(); i++) {
    selected_frames.row(i) = lbs_model.anim_frame_data.row(keyframes(i));
  }
  MatxXd selected_added_frames(keyframes.size(),
                               added_lbs_model.anim_frame_data.cols());
  for (int i = 0; i < keyframes.size(); i++) {
    selected_added_frames.row(i) =
        added_lbs_model.anim_frame_data.row(keyframes(i));
  }
  igl::writeDMAT(assets_path + "cheb_selected_anim.dmat", selected_frames);
  igl::writeDMAT(assets_path + "cheb_added_selected_anim.dmat",
                 selected_added_frames);
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    lbs_model.load_frame_data(frame);
    added_lbs_model.load_frame_data(frame);
    frame++;

    // aphys::MatxXd verts = lbs_model.lbs_weights * lbs_model.T;
    Vecxd T_vec = Vecxd::Map(lbs_model.T.data(), lbs_model.T.size());
    Vecxd verts_vec = lbs_model.lbs_weights_ext * T_vec;
    MatxXd verts = MatxXd::Map(verts_vec.data(), tm.verts.rows(), 3);
